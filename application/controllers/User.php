...
class user extends CI_Controller
{
  ...
  function all_cours(){
    ...
    $data['contact_info_site']  = $this->crud_model->Select_contact_info_site(); 
    $data['padding_articles']   = $this->crud_model->Select_padding_articles_tri();
    $this->load->view('backend/user/all_cours', $data);
  }
   ...
   */
   ...
   =================================
   *********************************
   script pour les cours 
   ==================================
   **********************************
   */

  // pagination de mes cours
  function pagination_access_our_cours()
  {

    $this->load->library("pagination");
    $config = array();
    $config["base_url"] = "#";
    $config["total_rows"] = $this->crud_model->fetch_pagination_cours();
    $config["per_page"] = 6;
    $config["uri_segment"] = 3;
    $config["use_page_numbers"] = TRUE;
    $config["full_tag_open"] = '<ul class="pagination">';
    ...
    $output = array(
     'pagination_link' => $this->pagination->create_links(),
     'country_table'   => $this->crud_model->fetch_details_pagination_cours($config["per_page"], $start)
    );
    echo json_encode($output);
  }
     ...
     // recherche de formations
     function fetch_search_our_cours()
     {
          $output = '';
          $query = '';
          if($this->input->post('query'))
          ...
          $data = $this->crud_model->fetch_data_search_allcours($query);
          ...
          if($data->num_rows() > 0)
          {
              ...
              foreach($data->result() as $key)
              {
                ...
                $link = '
                  ...
                  <a class="btn btn-hub btn-sm btn-block" href="'.base_url().'user/cours/'.$key->idf.'/'.$key->code.'"><i class="fa fa-hand-o-right"></i> Suivre le cours</a>';
                      ...
                      <div class="card-body">
                        <div class="col-md-12">
                          ...
                          <div class="row">
                            ...
                            <div class="col-md-12">
                              <a href="'.base_url().'user/cours/'.$key->idf.'/'.$key->code.'" class="img-fluid">
                                <img src="'.base_url().'upload/cours/'.$key->logo.'" alt="IMG" class="img img-responsive img-thumbnail" style="height: 200px; border-color:#DC4405;">
                              </a>
                            </div>

                            <div class="col-md-12">

                              <h5 class="card-title">'. substr($key->nomcours, 0,100).'</h5>
                             
                              <div class="card-text">
                                <i class="fa fa-calendar mr-1"></i> Mise à jour: <span class="text-warning">
                                   '.nl2br(substr(date(DATE_RFC822, strtotime($key->created_at)), 0, 23)).'
                                </span>
                              </div>
