   ...
   */
   ...
   function get_name_formation_pub($idf){
    ...
    function fetch_data_search_formations($query)
    {
     ...
     // recherche de articles
    function fetch_data_search_allcours($query)
    {
      $this->db->select("*");
      $this->db->from("profile_cours");
      $this->db->limit(8);
      if($query != '')
      {
       $this->db->like('nomcours', $query);
       $this->db->or_like('descriptioncours', $query);
     ...
     function fetch_details_view_mes_formations_user($id_user)
    {
     ...
     function fetch_pagination_cours()
    {
      $this->db->limit(100);
      $this->db->order_by('created_at', 'DESC');
      $query = $this->db->query("SELECT * FROM profile_cours");
      return $query->num_rows();
    }

     // detail des articles par profile_cours
    function fetch_details_pagination_cours($limit, $start)
    {
      $output = '';
      $this->db->select("*");
      $this->db->from("profile_cours");
